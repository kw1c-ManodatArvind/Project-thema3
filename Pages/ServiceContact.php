<!--
Auteur: Danijel Jovanovic
Datum: 9-4-2024
-->

<!DOCTYPE html>
<html lang="nl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Tui Website</title>
    <link rel="stylesheet" href="../styles/ServiceContact.css"> <!-- Koppeling naar CSS-bestand -->
</head>
<body>
<div class="navbar" style="background-color: #71cbf4;"> <!-- Navigatiebalk met blauwe achtergrond -->
    <img src="../images/logo.png" alt="Logo" class="logo"> <!-- TUI-logo -->
    <ul class="nav-links"> <!-- Navigatielinks -->
        <li><a href="../index.php">Home</a></li> <!-- Link naar Home -->
        <li><a href="resultaten.php">Resultaten</a></li> <!-- Link naar Resultaten -->
        <li><a href="ServiceContact.php">Service & Contact</a></li> <!-- Link naar Service & Contact -->
        <li><a href="Vliegtickets.php">Vliegtickets</a></li> <!-- Link naar Vliegtickets -->
        <li><a href="Vragenlijst.php">Vragenlijst</a></li> <!-- Link naar Vragenlijst -->
    </ul>
</div>
<img src="../images/HeaderPlaatje.jpg" alt="Header Image" class="header-img"> <!-- Headerafbeelding -->
<div style="text-align: center; margin-top: 20px;"> <!-- Centreren van tekst -->
    <h1 style="color: navy; font-size: 36px;">Service & contact</h1> <!-- Grote titel -->
    <a href="https://www.tui.nl/contact/"><img id="chatbot" src="../images/chatbot.png"></a> <!-- Afbeelding van chatbot met link -->
    <h1 style="color: navy; font-size: 36px;">Klik hierboven op de foto om het te vragen aan onze virtuele assistent</h1> <!-- Instructies voor gebruik van chatbot -->
</div>
<div class="decorative-links-container"> <!-- Container voor decoratieve links -->
    <div class="decorative-links"> <!-- Lijst van zelf regelen in myTUI -->
        <h2>Zelf regelen in myTUI</h2>
        <ul>
            <li><a href="#">Mijn boekingen bekijken</a> <span class="arrow">&#10132;</span></li>
            <li><a href="#">Mijn boekingen betalen</a> <span class="arrow">&#10132;</span></li>
            <li><a href="#">E-tickets downloaden</a> <span class="arrow">&#10132;</span></li>
        </ul>
    </div>

    <div class="decorative-links"> <!-- Lijst van zelf regelen TUI fly -->
        <h2>Zelf regelen TUI fly</h2>
        <ul>
            <li><a href="#">Online inchecken</a> <span class="arrow">&#10132;</span></li>
            <li><a href="#">Stoel reserveren</a> <span class="arrow">&#10132;</span></li>
            <li><a href="#">Bagage</a> <span class="arrow">&#10132;</span></li>
        </ul>
    </div>

    <div class="decorative-links"> <!-- Lijst van extra bijboeken -->
        <h2>Extra bijboeken</h2>
        <ul>
            <li><a href="#">Verzekeringen</a> <span class="arrow">&#10132;</span></li>
            <li><a href="#">Parkeren</a> <span class="arrow">&#10132;</span></li>
            <li><a href="#">Schiphol Lounge</a> <span class="arrow">&#10132;</span></li>
        </ul>
    </div>
</div>
<div class="more-help"> <!-- Meer hulp nodig-sectie -->
    <h2>Meer hulp nodig?</h2>
    <div class="help-options"> <!-- Opties voor hulp -->
        <div class="help-option"> <!-- Optie voor telefonische hulp -->
            <div class="icon">
                <!-- SVG code voor telefoon  -->
            </div>
            <p>Bel het TUI Customer Services Center.</p>
        </div>
        <!-- Andere opties voor hulp met iconen -->
        <div class="help-option"> <!-- Optie voor hulp via social media -->
            <div class="icon">
                <!-- SVG code voor tekstballon -->
            </div>
            <p>Social media<br>Stel je vraag aan ons social care team via Facebook of Twitter. Wij staan 7 dagen per week voor je klaar.<br>Ma t/m zat 08:00 - 20:00<br>Zondag 09:00 - 20:00</p>
        </div>
        <!-- Andere opties voor hulp met iconen -->
        <div class="help-option"> <!-- Optie voor bezoek aan reisbureau -->
            <div class="icon">
                <!-- SVG code voor pinnentje -->
            </div>
            <p>Reisbureaus en reisadvies<br>Bezoek één van onze 130 TUI Reisbureaus of neem contact op met een reisadviseur van TUIatHome.<br><a href="#">TUI reisbureaus</a><br><a href="#">Maak een afspraak</a><br><a href="#">TUIatHome</a></p>
        </div>
        <!-- Andere opties voor hulp met iconen -->
        <div class="help-option"> <!-- Optie voor hulp via TUI App -->
            <div class="icon">
                <!-- SVG code voor telefoon -->
            </div>
            <p>TUI App<br>Je complete vakantie in een app.</p>
        </div>
        <!-- Andere opties voor hulp met iconen -->
        <div class="help-option"> <!-- Optie voor indienen van klachten -->
            <div class="icon">
                <!-- SVG code voor potlood -->
            </div>
            <p>Klachten en suggesties<br>Is je reis niet volledig naar wens verlopen? Meld dit binnen twee maanden na terugkomst (volgens ANVR-voorwaarden).</p>
        </div>
        <!-- Andere opties voor hulp met iconen -->
        <div class="help-option"> <!-- Optie voor melden van vluchtvertraging -->
            <div class="icon">
                <!-- SVG code voor klok -->
            </div>
            <p>Vluchtvertraging TUI fly<br>Wil je een claim indienen voor een vertraging met TUI fly? Je kan je melding maken na de einddatum van je reis.</p>
        </div>
    </div>
</div>
<div class="about-tui"> <!-- Over TUI Nederland-sectie -->
    <h2>Over TUI Nederland</h2>
    <p class="social-media-header">Volg TUI op social media</p> <!-- Kop voor social media links -->
    <div class="social-media"> <!-- Social media links -->
        <a href="https://www.youtube.com/"><img src="../images/YoutubeLogo.jpg" alt="YouTube" width="100px"></a>
        <a href="https://www.instagram.com/"><img src="../images/InstaLogo.jpg" alt="Instagram" width="50px"></a>
        <a href="https://www.facebook.com/"><img src="../images/FacebookLogo.jpg" alt="Facebook"width="50px"></a>
        <a href="https://twitter.com/"><img src="../images/TwitterLogo.png" alt="Twitter" width="50px"></a>
    </div>
    <div class="about-tui"> <!-- Informatie over TUI Nederland -->
        <p>Hier vind je informatie over TUI Nederland, onze merken, werken bij TUI Nederland en onze.</p>
        <p class="address-info">
            Vestigingsadres TUI Nederland N.V.<br>
            Volmerlaan 3<br>
            2288 GC Rijswijk<br><br>

            Postadres:<br>
            Postbus 157<br>
            2280 AD Rijswijk<br><br>

            KvK nr.: 27148888<br>
            BTW nr.: NL804637295B01<br><br>

            IBAN: NL04 INGB 0661847535<br>
            BIC: INGBNL2A<br><br>

            TUI is een handelsnaam van TUI Nederland N.V. en TUI Airlines Nederland B.V.<br><br>

            TUI Airlines Nederland B.V<br>
            KvK nr.: 27098512<br>
            BTW nr.: NL 005200556B01<br><br>

            IBAN: NL04 INGB 0661847535<br>
            BIC: INGBNL2A
        </p>
    </div>

</div>
<div class="footer"> <!-- Footersectie -->
    <div class="footer-left">
        <h3>Goed voorbereid op reis</h3>
        <ul>
            <li><a href="#">TUI app</a></li>
            <li><a href="#">Online inchecken Transavia</a></li>
            <li><a href="#">Excursies op de bestemming</a></li>
            <li><a href="#">Goed voorbereid op reis</a></li>
            <li><a href="#">Verzekeringen</a></li>
            <li><a href="#">Autohuur</a></li>
            <li><a href="#">Parkeren op de luchthaven</a></li>
            <li><a href="#">Luchthavenvervoer</a></li>
            <li><a href="#">Visum & vaccinaties</a></li>
            <li><a href="#">Bekijk alle extra's</a></li>
        </ul>
    </div>
    <div class="footer-center">
        <h3>Vliegen met TUI fly</h3>
        <ul>
            <li><a href="#">Online inchecken TUI fly</a></li>
            <li><a href="#">Stoelreservering</a></li>
            <li><a href="#">Bagage</a></li>
            <li><a href="#">Bekijk alle TUI fly services</a></li>
            <li><a href="#">TUI Cargo</a></li>
        </ul>
    </div>
    <div class="footer-right">
        <h3>TUI Nederland</h3>
        <ul>
            <li><a href="#">Over TUI Nederland</a></li>
            <li><a href="#">Pers en media</a></li>
            <li><a href="#">Voorwaarden</a></li>
            <li><a href="#">Disclaimer</a></li>
            <li><a href="#">Copyright</a></li>
            <li><a href="#">Privacy & cookies</a></li>
            <li><a href="#">Klacht?</a></li>
            <li><a href="#">Deel een zorg</a></li>
        </ul>
    </div>
    <div class="footer-bottom">
        <div class="work-at-tui">
            <a href="#">Werken bij TUI</a>
        </div>
    </div>
</div>
<div class="newsletter"> <!-- Nieuwsbriefsectie -->
    <span>Nieuwsbrief</span>
    <p>Krijg wekelijks de beste deals in je mailbox.</p>
    <form action="#" method="POST">
        <input type="email" name="email" placeholder="Vul je e-mailadres in" required>
        <button type="submit">Aanmelden</button>
    </form>
    <div class="navbarOnder">
    </div>
</div>

</body>
</html> <!-- Einde van HTML-document -->
